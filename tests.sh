...
testProxies() {
  ...
  #check http proxy
  vpnIP=$(curl -m5 -sx http://${TCREDS}${PROXY_HOST}:${HTTP_PORT} "http://ipv4.lafibre.info/ip.php")
  if [[ ${myIp} != "${vpnIP}" ]] && [[ -n ${vpnIP} ]]; then
    echo "http proxy: IP is ${vpnIP}, mine is ${myIp}"
  else
    echo "Error, curl through http proxy to http://ipv4.lafibre.info/ip.php failed"
...
if [[ "localhost" == "${PROXY_HOST}" ]] && [[ 1 -eq ${BUILD} ]]; then docker compose -f ${CPSE} down; fi
