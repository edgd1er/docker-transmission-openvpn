...
if [[ -z ${json_countries} ]]; then
  ...
  json_countries=$(curl  ${nordvpn_api}/v1/servers/countries)
...
if [[ -z ${json_groups} ]]; then
  ...
  json_groups=$(curl  ${nordvpn_api}/v1/servers/groups)
...
if [[ -z ${json_technologies} ]]; then
  ...
  json_technologies=$(curl  ${nordvpn_api}/v1/technologies)
...
test4ServerName_given() {
  ...
  export NORDVPN_SERVER=$(curl 'https://api.nordvpn.com/v1/servers/recommendations?filters\[country_id\]=228&filters\[servers_technologies\]\[identifier\]=openvpn_tcp&limit=1' | jq -r .[].hostname)
...
select_hostname() { #TODO return multiples
  ...
  do
    vpnserver=$(curl "${nordvpn_api}/v1/servers/recommendations?${filters}limit=1" | jq --raw-output ".[].hostname")
    ((i++))
    if [[ ${i} -eq 5 ]];  then break; fi
    if [[ -z ${vpnserver} ]];  then
        log "Warning, ${i} trial(s) to get best server from nordvpn api."
    fi
    sleep 5
  done
  if [[ -z ${vpnserver} ]]; then
    ...
    load=$(curl ${nordvpn_api}/server/stats/${vpnserver} | jq .percent)
...
if [[ -n ${NORDVPN_SERVER} ]]; then
  ...
  load=$(curl ${nordvpn_api}/server/stats/${NORDVPN_SERVER} | jq .percent 2>/dev/null)
